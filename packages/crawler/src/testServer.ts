...
const PAGES: Record<string, (origin: string, host: string) => { status: number; headers?: Record<string, string>; body?: string }> = {
  '/': () => ({
    status: 200,
  ...
  // One path, two answers, chosen by the `Host` header: refused under a `www.`
  // host, served under the bare one. This is tartanhq.com's shape inverted so
  // it can be tested without DNS — see the crawlSite test for why the
  // inversion is what makes the test deterministic.
  '/host-split': (_origin, host) =>
    host.startsWith('www.')
      ? {
          status: 403,
          body: '<!doctype html><html><head><title>Blocked</title></head><body><h1>403</h1></body></html>',
        }
      : {
          status: 200,
          body: `<!doctype html><html><head><title>Served by the bare host</title>
            <meta name="description" content="The host that answered." />
          </head><body><h1>Home</h1><a href="/target">target</a></body></html>`,
        },
...
export function startTestServer(): Promise<TestServer> {
  return new Promise((resolve) => {
    const server: Server = createServer((req, res) => {
      ...
      const { status, headers = {}, body } = handler(origin, (req.headers.host ?? '').split(':')[0]);
