...
describe('detectContentIssues', () => {
  ...
  /**
   * The production failure of 2026-09-10: CloudFront's 403 page was stored as
   * tartanhq.com's homepage, and because it is real prose with a heading it
   * scored badly rather than not at all — reported as weak-eeat 0,
   * weak-entity-coverage 0 and sparse-internal-linking 0, which read as "this
   * customer's site is bad" when they meant "we were served an error page".
   */
  it('fires nothing for a non-2xx page, however much prose the error page has', () => {
    const blocked = page({
      statusCode: 403,
      title: 'ERROR: The request could not be satisfied',
      headings: [{ level: 1, text: '403 ERROR' }],
      bodyText: "Request blocked. We can't connect to the server for this app or website at this time. Generated by cloudfront (CloudFront)",
      internalLinkCount: 0,
    });
    const { issues, score } = detectContentIssues(blocked);
    expect(issues).toEqual([]);
    expect(score).toBeNull();

    // The same body at 200 is scored, so the silence is the status and not the
    // content — otherwise this test would pass with the guard removed.
    const served = detectContentIssues(page({ ...blocked, statusCode: 200 }));
    expect(served.issues.length).toBeGreaterThan(0);
  });
